    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {(prot.data?.accounts || []).map((a) => (
          <div key={a.account_id} className={`card space-y-2 ${a.green_api_deleted ? "border-red-200 bg-red-50" : a.in_cooldown ? "border-red-200 bg-red-50" : ""}`}>
            <div className="flex items-center justify-between">
              <span className="font-bold">{a.name}</span>
              {a.green_api_deleted
                ? <span className="badge bg-red-50 text-red-700 border-red-200">حذف‌شده در سرویس 🗑️</span>
                : a.in_cooldown ? <span className="badge bg-red-50 text-red-700 border-red-200">در خنک‌سازی تا {a.cooldown_until}</span>
                : a.throttle_factor < 1 ? <span className="badge bg-amber-50 text-amber-700 border-amber-200">کاهش سرعت ({pct(a.throttle_factor)})</span>
                : null}
            </div>
            {/* V36 — instance deleted upstream: terminal state + remove action ONLY (no health widgets) */}
            {a.green_api_deleted ? (
              <div className="space-y-2 text-sm">
                <p className="text-red-700 text-xs">{a.green_api_deleted_message || "این اینستنس در سرویس دیگر وجود ندارد"} — دیگر بررسی/اتصال ممکن نیست.</p>
                <button className="btn-danger btn-sm" onClick={async () => {
                  if (await confirmDialog("این حساب از پلتفرم حذف شود؟ (در سرویس از قبل حذف شده است)")) {
              ...
              <>
                <div className="text-xs text-muted">امتیاز سلامت<HelpTip text={TIPS.health} /></div>
                <Progress value={Math.round((a.health_score || 0) * 100)} max={100} color={a.health_score > 0.6 ? "bg-brand" : a.health_score > 0.3 ? "bg-amber-500" : "bg-red-500"} />
                ...
                {a.reply_rate_7d != null && a.reply_rate_7d < 0.2 && (
                  <p className="text-xs text-amber-700">⚠️ نرخ پاسخ پایین است — خطر مسدود شدن بالا می‌رود، حجم ارسال را کم کنید.</p>
                )}
                <div className="flex flex-wrap gap-1 pt-1">
                  <button className="btn-secondary btn-sm" disabled={a.in_cooldown}
      ...
      {inc.loading ? <Spinner /> : (!inc.data || inc.data.length === 0) ? <Empty label="رویدادی ثبت نشده." /> : (
        <div className="space-y-2">
          {inc.data.map((i) => (
            <div key={i.id} className={`card text-sm ${i.severity === "critical" ? "border-red-200" : "border-amber-200"}`}>
              <div className="flex items-center justify-between flex-wrap gap-2">
                <span className="font-bold">
                  {i.severity === "critical" ? "🔴" : "🟡"} {INCIDENT_FA[i.incident_type] || i.incident_type} — {i.account_name}
                </span>
                <span className="text-xs text-muted">{i.created_at} · {i.detected_via}</span>
              </div>
              ...
              {!i.resolved && (
                <button className="btn-secondary btn-sm mt-2" onClick={async () => { try { await IncidentsApi.resolve(i.id); toast.success("حل شد"); inc.reload(); } catch (e) { toast.error(e?.response?.data?.detail || e.message); } }}>
                  ✓ حل شد
                </button>
              )}
            </div>
