...
function OptOutSection() {
  ...
  return (
    <div className="card space-y-3">
      ...
      {loading && <Spinner />}
      {data && logs.length === 0 && <p className="text-sm text-muted">هنوز لغو خودکاری ثبت نشده.</p>}
      {logs.length > 0 && (
            ...
            <tbody>
              {logs.map((l) => (
                <tr key={l.id} className="border-b border-line">
                  ...
                  <td className="py-2 px-2 text-xs text-muted">{new Date(l.created_at).toLocaleString("fa-IR")}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {data?.map((b) => (
          <div key={b.id} className="card flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted">{b.reason || "بدون دلیل"} · {new Date(b.created_at).toLocaleDateString("fa-IR")}</p>
            </div>
            <button className="btn-danger btn-sm" onClick={async () => { await Api.remove(b.phone); reload(); }}>حذف از لیست</button>
          </div>
        ))}
      </div>
    </div>
