...
export default function Calls() {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted">⭐ تماس‌های بی‌پاسخ ورودی، سرنخ‌های داغ هستند — با آنها تماس بگیرید یا پیام بدهید.</p>
      ...
      {loading ? <Spinner /> : (!data || data.length === 0) ? <Empty label="تماسی ثبت نشده." /> : (
        <div className="table-wrap">
          <table className="table w-full text-sm">
            ...
            <tbody>
              {data.map((c) => (
                <tr key={c.id} className={`border-t border-line ${c.is_hot_lead ? "bg-red-50" : ""}`}>
                  ...
                  <td className={`p-2 ${c.is_hot_lead ? "text-red-700 font-bold" : ""}`}>{STATUS_FA[c.status] || c.status || "—"}{c.is_hot_lead && " 🔥"}</td>
                  <td className="p-2 text-xs text-muted">{c.called_at || "—"}</td>
                  <td className="p-2">
                    {c.from_phone && (
                      <div className="flex gap-1">
                        <a className="btn-secondary btn-sm" href={`tel:${c.from_phone}`}>تماس بگیرید</a>
                        <a className="btn-secondary btn-sm" href={`https://wa.me/${c.from_phone}`} target="_blank" rel="noreferrer">پیام بفرست</a>
                      </div>
                    )}
                  </td>
                </tr>
