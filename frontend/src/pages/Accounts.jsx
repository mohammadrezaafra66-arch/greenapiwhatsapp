...
// V18 PART 2 — Persian labels for the V17 warm-up enrollment state (accounts toggle badge).
const WARMUP_STATE_FA = {
  ENROLLED: "ثبت‌شده",
  COOLDOWN: "آماده‌سازی (۲۴س)",
  RECEIVING: "دریافت",
  REPLYING: "پاسخ‌دهی",
  ...
  GRADUATED: "آماده",
  PAUSED: "متوقف",
  YELLOWCARD: "استراحت (زردکارت)",
  BLOCKED_RESET: "بازنشانی",
};
                ...
                <input
                  ...
                  onChange={async (e) => {
                    try {
                      ...
                      if (e.target.checked) {
                        if (r?.notice) toast.error(r.notice);             // e.g. «اکانت گرم کافی نیست»
                        else toast.success("گرم‌سازی هوشمند فعال شد (مش V17)");
                      } else {
                        toast.success("گرم‌سازی متوقف شد");
                      }
                      await reload();
                    }
